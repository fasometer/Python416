...
from django.utils import timezone  # время
from django.contrib.auth.decorators import login_required  # protecton
...
def create_todo(request):
    if request.method == "GET":
        ...
        try:
            ...
            new_todo = form.save(commit=False)  # сохранени в преремнную
            new_todo.user = request.user
            new_todo.save()
            return redirect('currenttodos')
        except ValueError:
            return render(request, 'todo/createtodo.html', {"form": TodoForm(), 'error': "пререданы неверные данные"})
